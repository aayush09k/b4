...
class StunClient {
  ...
  Future<void> _processResponseIpv4() async {
    ...
    Datagram? datagram =
        await _socketIpv4!.receive(timeout: 5000); // Timeout set to 5000ms
  ...
  void _parseResponseIpv4(Uint8List response) {
    ...
    Uint8List magicCookie =
        response.sublist(magicCookieOffset, magicCookieOffset + 4);
  ...
  Future<void> _processResponseIpv6() async {
    ...
    Datagram? datagram =
        await _socketIpv6!.receive(timeout: 5000); // Timeout set to 5000ms
    if (datagram != null && datagram.data.length >= 20) {
      var response = datagram.data;
      if (response[0] == 0x01 && response[1] == 0x01) {
        // Binding Response Success
    ...
    while (index < 20 + messageLength) {
      ...
      if (type == 0x0001 || type == 0x0020) {
        ...
        if (family == 0x02) {
          ...
          List<int> xorKey = List.from(magicCookie)..addAll(transactionIDIpv6);
