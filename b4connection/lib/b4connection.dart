...
class B4connection{
  ...
  void localNodeListening()  {
    String? remoteIp;
    int? remotePort;

    try {
      Listening!.listen((socket)  {
        ...
        try {
          socket.listen(
              ...
              // Attempt to split the client message
              if (step == 0) {
                chatMode=true;
                ...
                if (parts.length == 4) {
                  ...
                  final name = parts[3];
                    if(action=='D') {
                      // Ensure the port is converted to an integer correctly
                      try {
                        remotePort = int.parse(parts[1]);
                        startConnection(remoteIp, remotePort);
                      } catch (e) {
                        print('Error parsing port to int: $e');
                      }
                    }
                    else{
                      Map<String?, dynamic> responseMap = {
                        "proxyIp": _publicIPv4,
                        "proxyPort": _publicPortIPv4,
                        "yourIp": ip,
                        "yourPort":port,
                        "myResponse": "G"
                      };
                  ...
                  print('Unexpected data format received: $clientMessage');
                }
              }
              else {
                print('remoteNode: $clientMessage');
  ...
  Future<void> startServerTcp()async{
    ...
    switch(layerID){
      case 0:{print('server is running for private nodes');try{Listening=await tcpClient.startServer();}
      catch(e){print('problem in socket');}
      break;}
      case 1:try{Listening=await tcpClient.startServer();}
      catch(e){print('problem in scoket');}break;
      case 2:try{Listening=await tcpClient.startServer();}
      catch(e){print('problem in socket');} break;
      case 3:Listening=await tcpClient.startServer(); tcpClient.startServer();break;
      case 4:try{Listening=await tcpClient.startServer();}
      catch(e){ print('problem in socket');} break;
      case null:print('layerID not defined');
    }
  ...
  Future<void>startConnection(targetIp,targetPort) async {
    ...
    if(reset==0){
      ...
      switch (layerID) {
        case 0:
          String toSend = "P|$_localIPv4|$_publicPortIPv4|macbook";
...
//You behaving as server can connect with other public node also you can help others to connect(Those are behind NAT) .
  Future<void> systemInformation() async {
    ...
    try {  await stunClient.initializeIpv4();
    ...
    Future<void> startSystem() async {
      if(_publicIPv6!=null) {
        ...
        switch(stunClient.NATcheckIpv4()){
          case true:{print('Not behind NAT in ipv4 system');layerID=1;startServerTcp(); break;}
          case false:{print('Behind NAT in ipv4system');layerID=0;startConnection(ipv4Pub,ipv4Port); }
        }
      }
    }
